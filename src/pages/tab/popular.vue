  ...
  <div class="popular">
    <el-tabs v-model="activeTabName"  @tab-click="handleTabClick">
      ...
      <el-tab-pane label="人气之选" name="popular">
        ...
        <el-button class="add" size="mini" type="primary" @click="tabPopularAdd">新增</el-button>
        <el-table border :data="tabPopular" class="table" v-loading="loading" element-loading-text="拼命加载中">
          ...
          <el-table-column label="操作"  align="center">
            <template slot-scope="scope" class="operate">
              <el-button type="danger" size="mini" @click="tabPopularDel(scope.$index,scope.row)">删除</el-button>
              <el-button size="mini" @click="tabPopularMove(scope.$index,scope.row,'top')">置顶</el-button>
              <el-button size="mini" @click="tabPopularMove(scope.$index, scope.row,'up')">上移</el-button>
              <el-button size="mini" @click="tabPopularMove(scope.$index,scope.row,'down')">下移</el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-tab-pane>
    ...
    <fundAdd-edit2 :type="tabName" :selectedFunds="tabPopular"  :edit="editData" :visible="editVisible" @close="closeEdit" @add="addFund"></fundAdd-edit2>
  </div>
</template>
  ...
  export default {
    ...
    data(){
      return {
        ...
        fundType:'货币型',
        type:2, //1.首页  2.标签页
        module:5,//首页   1热门产品、2指数宝、3基金精选、4人气之王、5热门关注
    ...
    methods:{
      ...
      updateList(){
        ...
        this.$http.get(api.getPageFundList + `?${getParams(params)}`).then(res=> {
          if (solve_res(res, "", "数据获取失败")) {
            this.tabPopular = res.results || [];
            for (var i = 0; i < this.tabPopular.length; i++) {
              this.tabPopular[i].ratetype = changeNumToCN(this.tabPopular[i].ratetype);
            }
          }
        this.loading = false;
        }).catch(err=> {
      ...
      tabPopularMove(index,row,operate){
        var params = {
        ...
        var end = Number(this.tabPopular[this.tabPopular.length-1].sortflag);
        if(operate == 'top'){
