...
//        return newVal
    ...
    methods: {
      updateFunds(){
        ...
        this.$http.get(api.getFundList+`?${getParams(params)}`).then(res=>{
          ...
          for (var i = 0; i < this.funds.length; i++) {
            var obj = this.funds[i];
            console.log("fundtype:",obj.fundtype);
          }
          this.checkFundList();
        }).catch(err=>{
            console.log(err);
      ...
      querySearchAsync(queryString) {
        ...
        var results =  funds.filter(this.createStateFilter(queryString,this.form.fundtype));

//          var results = queryString ? funds.filter(this.createStateFilter(queryString,this.form.fundtype)) : funds;
        for (var i = 0; i < funds.length; i++) {
          funds[i].value = funds[i].fundname;
        }
        return results;
      },
      createStateFilter(queryString,fundtype) {
        return (state) => {
          if(fundtype!=undefined){
            if(state.fundtype != fundtype){
              return false;
            }
          }
          if(queryString){
      ...
      submit(){
        ...
        if (this.form.fundcode) {
          confirm({title: '确定添加该基金？', confirm_btn: '添加'}, ()=> {
