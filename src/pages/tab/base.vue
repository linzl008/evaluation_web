<template>
  <div class="base">
    <el-tabs v-model="activeTabName"  @tab-click="handleTabClick">
      ...
      <el-tab-pane label="优选货基" name="base">
        <el-button class="add" size="mini" type="primary" @click="tabBaseAdd">新增</el-button>
        <el-table border :data="tabBase" class="table" v-loading="loading" element-loading-text="拼命加载中">
          ...
          <el-table-column label="操作"   align="center">
            <template slot-scope="scope" class="operate">
              <el-button type="danger" size="mini" @click="tabBaseDel(scope.$index,scope.row)">删除</el-button>
              <el-button size="mini" @click="tabBaseMove(scope.$index,scope.row,'top')">置顶</el-button>
              <el-button size="mini" @click="tabBaseMove(scope.$index, scope.row,'up')">上移</el-button>
              <el-button size="mini" @click="tabBaseMove(scope.$index,scope.row,'down')">下移</el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-tab-pane>
    ...
    <fundAdd-edit2 :type="tabName" :selectedFunds="tabBase"  :edit="editData" :visible="editVisible" @close="closeEdit" @add="addFund"></fundAdd-edit2>
  </div>
</template>
...
//        fundtype:''
        ...
        this.$http.get(api.getPageFundList+`?${getParams(params)}`).then(res=>{
          if(solve_res(res,"","数据获取失败")){
            this.tabBase =res.results || [];
            for (var i = 0; i < this.tabBase.length; i++) {
              this.tabBase[i].ratetype = changeNumToCN(this.tabBase[i].ratetype);
            }
          }
          this.loading = false;
        }).catch(err=>{
      ...
      tabBaseMove(index,row,operate){
        ...
        var end = Number(this.tabBase[this.tabBase.length-1].sortflag);
        if(operate == 'top'){
