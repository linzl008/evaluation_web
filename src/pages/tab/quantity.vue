<template>
  <div class="quantity">
    <el-tabs v-model="activeTabName"  @tab-click="handleTabClick">
      <el-tab-pane label="优选量化" name="quantity">
        <el-button class="add" size="mini" type="primary" @click="tabQuantityAdd">新增</el-button>
        <el-table border :data="tabQuantity" class="table" v-loading="loading" element-loading-text="拼命加载中">
          ...
          <el-table-column label="操作"  align="center">
            <template slot-scope="scope" class="operate">
              <el-button type="danger" size="mini" @click="tabQuantityDel(scope.$index,scope.row)">删除</el-button>
              <el-button size="mini" @click="tabQuantityMove(scope.$index,scope.row,'top')">置顶</el-button>
              <el-button size="mini" @click="tabQuantityMove(scope.$index, scope.row,'up')">上移</el-button>
              <el-button size="mini" @click="tabQuantityMove(scope.$index,scope.row,'down')">下移</el-button>
            </template>
          </el-table-column>
    ...
    <fundAdd-edit2 :type="tabName" :selectedFunds="tabQuantity"  :edit="editData" :visible="editVisible" @close="closeEdit" @add="addFund"></fundAdd-edit2>
  </div>
</template>
...
//        fundtype:''
        ...
        this.$http.get(api.getPageFundList+`?${getParams(params)}`).then(res=>{
          if(solve_res(res,"","数据获取失败")){
            this.tabQuantity=res.results || [];
            for (var i = 0; i < this.tabQuantity.length; i++) {
              this.tabQuantity[i].ratetype = changeNumToCN(this.tabQuantity[i].ratetype);
            }
          }
          this.loading = false;
        }).catch(err=>{
      ...
      tabQuantityMove(index,row,operate){
        ...
        var end = Number(this.tabQuantity[this.tabQuantity.length-1].sortflag);
        if(operate == 'top'){
