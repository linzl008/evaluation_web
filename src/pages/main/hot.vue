  ...
  <div class="hot">
    <el-tabs v-model="activeMainName"  @tab-click="handleTabClick">
      ...
      <el-tab-pane label="热门关注" name="hot">
        <el-button class="add" type="primary" size="mini" @click="mainHotAdd">新增</el-button>
        <el-table border :data="mainHot" class="table" v-loading="loading" element-loading-text="拼命加载中">
          ...
          <el-table-column label="操作"  prop="title" align="center">
            <template slot-scope="scope" class="operate">
              <el-button type="danger" size="mini" @click="mainHotDel(scope.$index,scope.row)">删除</el-button>
              <el-button size="mini" @click="mainHotMove(scope.$index,scope.row,'top')">置顶</el-button>
              <el-button size="mini" @click="mainHotMove(scope.$index, scope.row,'up')">上移</el-button>
              <el-button size="mini" @click="mainHotMove(scope.$index,scope.row,'down')">下移</el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-tab-pane>
    </el-tabs>
    <fundAdd-edit :type="tabName" :selectedFunds="mainHot" :edit="editData" :visible="editVisible" @close="closeEdit" @add="addFund"></fundAdd-edit>
  </div>
</template>
...
//        fundtype:''
      ...
      this.$http.get(api.getPageFundList+`?${getParams(params)}`).then(res=>{
        if(solve_res(res,"","数据获取失败")){
          this.mainHot=res.results || [];
          for (var i = 0; i < this.mainHot.length; i++) {
            this.mainHot[i].ratetype = changeNumToCN(this.mainHot[i].ratetype);
          }
        }
        this.loading = false;
      }).catch(err=>{
    ...
    mainHotMove(index,row,operate){
      ...
      var end = Number(this.mainHot[this.mainHot.length-1].sortflag);
      if(operate == 'top'){
